    ...
    if FLASH_ATTN_2_AVAILABLE or FLASH_ATTN_3_AVAILABLE:
        ...
        try:
            ...
            if s_q == s_k:  # skip for cross-attention where seq lens differ
                scale = d ** -0.5
                # [B, S, n, d] → [B*n, S, d], q pre-scaled
                q_3d = q.to(dtype).permute(0, 2, 1, 3).reshape(b * n, s_q, d).contiguous() * scale
                # k transposed: [B*n, d, S_k]
                k_3d = k.to(dtype).permute(0, 2, 3, 1).reshape(b * n, d, s_k).contiguous()
                v_3d = v.to(dtype).permute(0, 2, 1, 3).reshape(b * n, s_k, d).contiguous()
